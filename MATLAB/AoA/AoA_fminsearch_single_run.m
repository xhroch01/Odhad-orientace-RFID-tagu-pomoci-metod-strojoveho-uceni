...
fileName = 'C:\Sim\Dataset_tag_test\AoA_LHS_dataset\lambda3\AoA_lambda3_X_cs_14_Y_cs_374_RotXY_m77_RotXZ_0_RotYZ_0.fld'; % NUTNO SPRAVNE NASTAVIT ROZESTUPY SOND d_ref!!
...
for i = 1:length(readers)
    ...
    text(centerY + 0.1, centerX, sprintf('Čtečka %d', i), 'Color', 'blue', 'FontSize', 25, 'FontWeight','bold');
    ...
    line_icon = plot([centerY, endY], [centerX, endX], 'g-', 'LineWidth', 3);
end

% Chybová úsečka
error_line = plot([tY, cY], [tX, cX], 'm--', 'LineWidth', 2.5);

% Vypočítaná poloha tagu
calculated_position = plot(cY, cX, 'ko', 'MarkerSize', 15, 'LineWidth', 2, 'MarkerFaceColor', 'black');
...
xlim([-1.5, 1.5]); 
ylim([-1, 1.25]);
xlabel('Osa Y [m]', 'FontSize', 25);
ylabel('Osa X [m]', 'FontSize', 25);

% Titulek
title({['AoA Fminsearch: ', strrep(displayName, '_', '\_')], ...
       sprintf('Chyba: %.1f mm', distance_error * 1000)}, 'FontSize', 25);
