...
for i = 1:length(readers)
    ...
    % Čtečka
    reader_icon = plot(rY, rX, 'bs', 'MarkerSize', 20, 'MarkerFaceColor', 'blue');
    text(rY + 0.1, rX, sprintf('Čtečka %d', i), 'Color', 'blue', 'FontSize', 25, 'FontWeight', 'bold');
    
    % Čára AoA
    line_icon = plot([rY, rY+10*cos(az)], [rX, rX+10*sin(az)], 'g-', 'LineWidth', 3);
end
...
% Vypočítaná poloha
calculated_pos = plot(cY, cX, 'ko', 'MarkerSize', 15, 'LineWidth', 2, 'MarkerFaceColor', 'black');
